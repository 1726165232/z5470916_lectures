...
def get_tics(pth):
    ...
    with open(pth, 'r') as file:
        for line in file:
            ...
            parts = line.split('=')
            if len(parts) == 2:
                ticker, exchange = parts[1].strip().lower(), parts[0].strip().lower()
                ticker, exchange = ticker.replace('"', ''), exchange.replace('"', '')
...
def line_to_dict(line):
    ...
    """
    # <COMPLETE THIS PART>
    dict = {column:None for column in COLUMNS}

    start = 0
    for column in COLUMNS:
        dict[column] = line[start:start + COLWIDTHS[column]]
        start += COLWIDTHS[column]
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    # <COMPLETE THIS PART>
    if tickers_lst != None:
        if len(tickers_lst) == 0:raise Exception('The input list is empty')

        for element in tickers_lst:
            if element not in tic_exchange_dic:
                raise Exception(element+' is not a key in tic_exchange_dic')
...
def verify_cols(col_lst=None):
    ...
    # <COMPLETE THIS PART>
    if (col_lst != None):
        if len(col_lst) == 0:raise Exception('The input list is empty')

        for element in col_lst:
            if element not in COLUMNS:
                raise Exception('The name '+element+" is not found in COLUMNS")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    verify_cols(col_lst)
    final_dict = {tic: None for tic in tickers_lst}

    for tic in tickers_lst:
        lines = read_dat(tic)
        info = {}
        info['exchange'] = tic_exchange_dic[tic]
        data_lst = []

        for line in lines:
            data = line_to_dict(line)
            data_lst.append({key: data[key] for key in col_lst})
        info['data'] = data_lst
